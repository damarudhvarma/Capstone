...
| **User Registration via Email** | Full account creation flow from Sign In → Create Your Account → fill form → verify My Account page |
...
| SUP-02 | **Registration with mandatory fields empty** — submits blank form and verifies validation messages are displayed | Scenario |
...
| ID | Scenario | Tag |
|---|---|---|
| PS01 | **Verify user registration using email** — Sign In → Create Your Account → fill details → verify My Account page | `@Registration` |
...
| PS04 | **Complete Guest Checkout** — search "tshirt" → add to cart → checkout → fill guest personal info, address, shipping, payment → verify order placed | `@GuestCheckout` |
...
- Two parallel Page Object sets exist (`src/main/java/pages` and `src/test/java/prestapages`) — the latter is an extended set that includes newer page objects for Contact Us, Search and Add to Cart, Multiple Products, and Sort Products alongside earlier variants.
...
- Scenario tags (`@Registration`, `@ProductSearch`, `@GuestCheckout`, `@prestashop`, `@checkoutFlow`) can be used with `mvn test -Dcucumber.filter.tags="@TagName"` to run a filtered subset of tests.
